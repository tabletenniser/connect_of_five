 ...
 */
public class interPanelTwoPlayers extends JPanel {
					...
					if (chessBoard[x][y]==0){
						...
						if (clickCount%2==0){
							...
							if (OnePlayer.IsBlackWin(chessBoard)){
								...
								JFrame curFrame=(JFrame)(SwingUtilities.getRoot(interPanelTwoPlayers.this));
								curFrame.dispose();
								return;
							}
							clickCount++;
						}else if (clickCount%2==1){
							chessBoard[x][y]=-1;	//white move

							// if white wins, go to MainProgram
							if (OnePlayer.IsWhiteWin(chessBoard)){
								JOptionPane.showMessageDialog(null,"WHITE WINS!!!","Game Finished",JOptionPane.INFORMATION_MESSAGE);
								int response=JOptionPane.showConfirmDialog(null,"Do you want to save this game?","confirmation",JOptionPane.YES_NO_OPTION);
	...
	private void createAndWriteToData(){
		...
		dataFile	=new File("./StoredGamesForConnectOfFive/"+
				TwoPlayers.playerOne+"_vs_"+TwoPlayers.playerTwo+".dat");
		try{
